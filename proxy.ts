/**
 * Next.js Proxy (formerly Middleware) - Runs on EVERY request before reaching pages/routes
 *
 * PURPOSE: Refresh Supabase auth sessions automatically using SSR pattern
 *
 * WHY THIS EXISTS:
 * - Supabase uses JWT tokens that expire after 1 hour
 * - Without this proxy, users would be randomly logged out
 * - Proxy refreshes the session cookie on every request automatically
 ...
 * Supabase's SSR pattern requires proxy middleware to manage session cookies.
